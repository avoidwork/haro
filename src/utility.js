...
export function concatURI (left, right) {
	return left.replace(regex.querystring, "").replace(regex.endslash, "") + (right ? "/" + right : "");
}

export function keyIndex (key, data, delimiter, pattern) {
	let result;
...
export function delIndex (index, indexes, delimiter, key, data, pattern) {
	index.forEach(i => {
...
export function joinData (id, a, b, key, on, type = "inner") {
	const result = [];
...
export function createPatch (ogdata = {}, data = {}, key = "", overwrite = false) {
	const result = [];
...
export function setIndex (index, indexes, delimiter, key, data, indice, pattern) {
...
export function toObjekt (arg, frozen = true) {
	const result = {};
