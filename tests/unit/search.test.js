	...
	describe("matchesPredicate() complex array logic", () => {
		it("should handle array predicate with array value using AND logic", () => {
			...
			assert.equal(
				result2,
				false,
				"Should not match when not all predicate values are in record array",
			);
		});
		...
		it("should handle array predicate with array value using OR logic", () => {
			...
			assert.equal(
				result,
				true,
				"Should match when at least one predicate value is in record array",
			);
		});
		...
		it("should handle array predicate with scalar value using AND logic", () => {
			...
			const result2 = testStore.matchesPredicate(
				record,
			...
			assert.equal(
				result2,
				false,
				"Should not match when predicate array doesn't contain scalar value",
			);
		});
