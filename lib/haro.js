			...
			value: function del(key) {
				...
				return defer.promise.then(function (arg) {
					var next = function next(err) {
						...
						if (og) {
							_this3.set(key, og).then(function () {
								if (_this3.logging) {
					...
					if (og && _this3.uri && !batch) {
						if (_this3.patch) {
							_this3.request(concatURI(_this3.uri, null), { method: "patch", body: JSON.stringify([{ op: "remove", path: "/" + key }], null, 0) }).then(next, function (e) {
								if (e[1] === 405) {
									_this3.patch = false;
									_this3.request(concatURI(_this3.uri, key), { method: "delete" }).then(null, next);
								}
							});
						} else {
							_this3.request(concatURI(_this3.uri, key), { method: "delete" }).then(null, next);
				...
				return defer.promise.then(function (arg) {
					...
					if (!batch && _this13.uri) {
						_this13.transmit(key, x, og, override, method).catch(function (e) {
							...
							_this13.set(key, og).then(function () {
								if (_this13.logging) {
									console.log("Reverted", key);
								}
							}).catch(function () {
