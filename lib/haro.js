		...
		_createClass(Haro, [{
			...
			value: function request(input) {
				...
				fetch(input, cfg).then(function (res) {
					...
					res[res.headers.get("content-type").indexOf("application/json") > -1 ? "json" : "text"]().then(function (arg) {
						defer[status < 200 || status >= 400 ? "reject" : "resolve"](tuple(_this9.onrequest(arg, status, headers), status, headers));
					}, function (e) {
								...
								switch (true) {
									...
									case lkey === value:
										lset.forEach(function (key) {
											if (!seen.has(key)) {
												seen.add(key);
												result.push(_this11.get(key, raw));
											}
										});
										break;
									default:
			...
			value: function set(key, data) {
				...
				var next = function next(arg) {
					...
					if (lkey === null) {
						if (_this12.key) {
							xdata = _this12.crawl(xdata);
							lkey = xdata[_this12.key] || ldata[_this12.key] || uuid();
						} else {
							lkey = uuid();
						}
					}
					...
					if (method === "post") {
						_this12.registry[_this12.total] = lkey;
						++_this12.total;

						if (_this12.versioning) {
							_this12.versions.set(lkey, new Set());
						}
					} else {
						if (_this12.versioning) {
							_this12.versions.get(lkey).add(tuple(ogdata));
						}

						delIndex(_this12.index, _this12.indexes, _this12.delimiter, lkey, ogdata, _this12.pattern);
					}

					_this12.data.set(lkey, ldata);
					setIndex(_this12.index, _this12.indexes, _this12.delimiter, lkey, ldata, null, _this12.pattern);
					defer.resolve(_this12.get(lkey));

					if (!lload) {
						_this12.storage("set", lkey, ldata).then(function (success) {
